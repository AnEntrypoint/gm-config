...
`agentplug-liqology` (repo `AnEntrypoint/liqology`, submoduled at `liqology/` alongside `agentplug-bert`/`agentplug-libsql`/`agentplug-treesitter`) is an optional third-party agentplug plugin, NOT part of gm's own compiled default -- reachable only from a project whose `.agentplug/capability-allowlist.json` grants the calling plugin `liqology` access (least privilege, per Saltzer & Schroeder, matching `imports.rs`'s `compiled_default_capability_allowlist` gm/bert/libsql/treesitter-only default). It wraps a real, from-source-vendored FAISS `IndexFlatIP` (compiled for `wasm32-wasip1-threads`, `-fno-exceptions` since this wasi-sdk's prebuilt `libc++abi` lacks a working exception runtime for that triplet) behind five verbs: `record` (embed an interaction's input/output, reinforce FAISS-similar prior entries, decay and prune the rest per a `CostBalancePolicy`), `query_relevance`, `prune_report` (a real preview -- what would be evicted under the current policy, without mutating state), `tune_policy`, `capabilities`.

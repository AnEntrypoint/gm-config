...
**Prove before you test; the gate predicates make this structural, not aspirational.** VERIFY now refuses a diff carrying an admit/deferral marker (TODO/FIXME/XXX/HACK/'not (yet) implemented'), a high-confidence secret literal, a bare unwrap/expect/panic!/throw-without-handling outside a test path, a hedge phrase in touched prose, a decorative non-ASCII glyph, or a non-idempotent replayed dispatch tuple -- each is a real compiled gate on the VERIFY -> CONSOLIDATE edge, firing its own `deviation.*` naming the exact lines. These stand in for the full formal-methods bar (dependent types, total pure functions, affine state safety, happens-before, zero-trust authz) by enforcing the mechanically-checkable subset at the gate and leaving the rest to the adversarial sweep below, which you must actually run, not describe.

**Total-function and affine-state sweep, same discipline as the corner-case sweep above.** For every new function in the diff: confirm it returns on every input path (no implicit undefined/None on an unhandled branch), that every resource it takes ownership of is dropped exactly once (no leak, no double-free, no use-after-move in Rust, no un-closed handle/stream in JS), and that no hidden mutation escapes its declared effect boundary (a "pure" fn that secretly writes module state, a get that mutates). Witness each by exercising the edge inputs live via `exec_js`, never by re-reading the signature.

**Happens-before and disjointness sweep.** For every new concurrent access in the diff: confirm the two operations are ordered by an explicit sync point (await, lock, channel, atomic) or provably touch disjoint state -- no shared mutable cell touched from two async paths without one. Witness by interleaving the two calls under `exec_js` with a deterministic seed and asserting the outcome is ordering-invariant.

**Decisive-commitment sweep.** Re-read every new `.md`/`.txt`/comment-bearing file the diff touched: no hedge ('we should probably', 'for now', 'as a stopgap', 'out of scope for this'), no infinite option listing in place of a recommendation, no rationalized abandonment of a row that was actually completable. The gate's `no-hedge-language-in-diff` catches the common phrases; this sweep catches the shape the phrase-list misses.

**Unbounded-effort and real-termination sweep.** Before accepting convergence, confirm no row was closed with an artificial ceiling ('good enough for now', a partial emit shipped because 'the rest is out of scope'), and no completable goal was abandoned under a manufactured blocker (a tool crash never driven past, a credential never scripted around, a cross-repo change never opened). Each is a `prd-add` + real execution, not a narrated justification.

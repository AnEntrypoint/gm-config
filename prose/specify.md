...
## Supply-chain scan (dependency trust boundary, checked once per session touching dependencies)

`scan_deps` (rs-plugkit's `Capability::ProjectPath` verb) scans the project's git-tracked source plus any `node_modules` present for the "HiddenSpawn"-class obfuscated dropper (confirmed across 17+ separately-compromised repos, 2026-08): a source file gets a payload appended after its real end -- one extremely long, whitespace-padded line resolving a C2 address, fetching, decoding, and `eval`/`spawn`-ing attacker code. Two structural checks survive the next variant's changed C2/IP/wallet/cipher, never today's literal sample values: (1) byte size wildly disproportionate to line count, (2) a dense run (4+) of `\uXXXX` escapes decoding to an identifier shape (letters/digits/underscore, starting with a letter) -- real code never escapes an ASCII identifier this way, an attacker does it to dodge a plain-text grep for `require`/`spawn`/`child_process`. Body `{}` scans the whole project; `{"root": "<relative-dir>"}` scopes the git-tracked-source half to a subdirectory (`node_modules` always resolves at the project root regardless of `root`). `node_modules` walks per-package against a changed-since-last-scan stamp (`.gm/scan-deps-stamp.json`, package mtime+size) plus a noise-dir/noise-suffix ignore list (test/docs/examples/fixtures, `.map`/`.d.ts`/`.md`), so an unchanged package is skipped on every dispatch after the first and a stable tree stays fast. `{"full": true}` forces a full re-walk ignoring the stamp (a genuinely exhaustive one-off sweep, e.g. right after a suspicious install -- not the per-session default). Response `data`: `ok` (bool), `failCount`/`warnCount`/`blockedCount`, `failing`/`warnings`/`blocked` arrays (`path` plus detail fields), `nodeModulesTruncated` (bool -- a large tree the scan did not fully cover, disclosed never silent).

Dispatch `scan_deps` on any project's first `npm install`/dependency-install this session, and before trusting any freshly-cloned/updated `node_modules` or vendored dependency content. `blockedCount > 0` is itself evidence, not noise to route around -- a blocked read is the AV/OS itself already flagging the file. `failCount > 0` is a real hit. `warnCount > 0` alone (size-ratio disproportion, no escape-density corroboration) is usually a legitimate minified/bundled dependency -- worth a glance, never a block. `nodeModulesTruncated: true` with no standing unbounded scanner of the project's own for a less-frequent full sweep -> `prd-add` a row to add one.

A real hit (`failCount > 0` or `blockedCount > 0`) is a world-scope one-way-door concern: surface it to the user immediately via `AskUserQuestion`, never silently work around it (no exclusions, no blind retries, no "it's probably a false positive"). Investigate via the dependency's own git history/GitHub API (bypasses local AV blocks cleanly) to find the exact introducing commit and confirm the last clean one before proposing a fix (pin/revert/exclude). Several unrelated repos under one account/org showing the same pattern points at a compromised credential (an org-level token, a shared release-workflow secret) more than independent attacks -- name that possibility to the user rather than only fixing files one at a time. Fixing a compromised GitHub-sourced dependency's own `main` (not just the local install) needs the user's explicit go-ahead before any push -- prefer `git revert` (keeps the compromised commit visible as evidence) over a history rewrite unless the user explicitly asks for the stronger squash/rewrite.
...
## Scope discovery and fan-out

Discovery does not stop at the first plan. A closure that implies work outside the current PRD is a scope expansion, not a note: `prd-add` the new rows in the same pass, derive their own mutables in turn, and loop. This is not the barred "new run" (Section 2's "Maximum effort per run" bars unrelated work) -- scope discovered inside the same request's closure is not unrelated. The only thing that licenses leaving cover is a sweep that adds zero new mutables and zero new PRD rows: a discovery fixed point, not a step count or a feeling of coverage.

Multi-session is a default shape for long-horizon work, not a fallback for running out of room. State lives on disk (`.gm/prd.yml`, `.gm/mutables.yml`, `.turn-summary.json`) precisely so a fresh session's boot probe resumes the same walk with no replay and no re-derivation from memory. Once a batch of rows is independent enough to run unattended, hand it to a new session deliberately rather than serializing everything through one context.

Multi-agent fan-out is the default shape within a session for every batch of independent rows (entry.md's "Subagent fan-out" covers the SESSION_ID-minting mechanics; this is when and how to decompose). A shared recurring transform -- a rename sweep, an API migration, a bulk lint-class fix, N>5 applications of one pattern -- gets one reviewed mapping before fan-out, not N independent interpretations: a single subagent first drafts a mapping/edge-case note for the pattern (old shape to new shape, the corner cases it must preserve), a second subagent adversarially reviews that note (find the cases it misses or the conflicting instructions it gives -- the same refute-only posture DECIDE's own sweep takes toward code), and only then do the parallel workers dispatch, each referencing the reviewed mapping instead of inventing its own reading. Skipping this for a shared pattern is how N parallel agents land N subtly different handlings of the same edge case.

A large classifiable finding-set (compiler errors, lint violations, a dependency-bump breakage) is partitioned once, never re-classified mid-fan-out. Run the classifying dispatch (a build, a lint pass, whatever produces the finding list) a single time, group the output by its natural boundary (file, crate, module), and turn each partition into one PRD row owned by one subagent -- disjoint slices need no coordination. Each subagent verifies its own slice by re-running the classifier scoped to its own files, never the full classification again; re-running the global classifier mid-fan-out either wastes the run or risks two agents racing to fix the same already-reported finding.

Every dispatch is goal-oriented. A subagent's or sub-session's prompt states the terminal condition it serves -- `prd_pending_count=0` against the full discovered scope, not just its own slice. A dispatch that surfaces a new mutable mid-task feeds it back into `mutable-add`/`prd-add` instead of quietly narrowing scope to fit what it was told.
...
`prd-add` takes `id` -- kebab-case slug (`dedupe-update-error`). Always pass it explicitly. Omitting `id` is NOT silently auto-generated: the handler tries to derive a slug from `subject`/`title`/`name`/`task`/`goal`/`description`/`notes`, and if none of those yield usable text either, the call is HARD-REJECTED (`deviation.prd-add-no-id`, no row written) -- retrying the identical no-id call repeats the same rejection forever, burning turns. On rejection: add `id` directly, or add one of those text fields, then re-dispatch. Upsert semantics, three distinguishable outcomes: a fresh id appends (`{"added": id}`); an existing id whose stored row DIFFERS from the body is rewritten in place (`{"rescoped": id}`) preserving position/dependents -- the re-scope path on a reshaping discovery; an existing id whose stored row is already identical to the body reports `{"added": id, "already_identical": true}`, because nothing was re-scoped. So `rescoped` now means a real reshape and nothing else -- not a CAS retry re-reading a doc that already carries this same call's earlier attempt, and not a re-dispatch of an identical body after the first dispatch's out-file never reached you. Never delete-and-re-add (orphans the handle). Re-entry to SPECIFY is first-class, not failure -- the graph's feedback edges (every later stage -> SPECIFY) exist for exactly this.

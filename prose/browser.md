...
Three verbs share the CDP-capable browser family, one plain-text-body
grammar: `serp` runs a headless pure-Rust engine (oxibrowser) in-process --
fast, no Chrome process, but only navigate/evaluate/dom-query/
extract-markdown -- no screenshot/capture/profile/trace/viewport, and one
implicit session per instance (`session new/close/reset` are accepted
no-ops). `browser` dials a real CDP-speaking engine (lightpanda by default,
or steel-browser when `.gm/browser-config.json` names its endpoint) --
everything named in this document -- session-per-Chromium,
`screenshot`/`capture`/`profile`/`trace`, headed-by-default, CDP recipes --
applies to `browser` and to `cdp` identically, since both drive the same
CDP-over-port session machinery, differing only in which engine answers the
port (`browser`: lightpanda/steel; `cdp`: a locally-spawned real Chrome).
Any visual/rendering witness (the Hard Rule below) REQUIRES `screenshot`
mode on `browser` or `cdp`; `serp` cannot satisfy it and will reject a
`screenshot`/`capture`/`profile`/`trace` prefix with a note pointing at
`cdp`/`browser`. For a plain evaluate/navigate/dom-query witness where
visual fidelity does not matter, `serp` is the fastest, in-process default
-- reach for `browser` or `cdp` whenever the task needs anything this
paragraph names that `serp` does not cover; reach for `cdp` specifically
when the task needs real Chrome fidelity (a devtools-dependent site, exact
layout/rendering parity) that lightpanda/steel cannot guarantee.

## Hard Rule: Browser Witness Mandate

**Every edit to browser-run code requires a live `serp`, `browser`, or `cdp` dispatch, same turn as the edit.** Client-side surfaces -- `.html`, `.js`, `.jsx`, `.ts`, `.tsx`, `.vue`, `.svelte`, `.mjs`, `.css`, web components, service workers, every `<script>`-loaded asset, every `import`-reached path from a browser entry -- witnessed by live `page.evaluate` of the specific invariant the edit establishes; use `cdp` where the invariant needs real Chrome fidelity or a screenshot, `browser` for a CDP-capable witness without spawning local Chrome, `serp` (oxibrowser, in-process) otherwise. Passing node test / build / `curl` of HTML / static-analysis witnesses server delivery, not browser behavior -- non-substitutive. The witness IS the proof; prose is not.
...
**A dispatch that RAN is not the same claim as a dispatch that PASSED -- a negative result is a fail, not a witness.** The harness marks a file witnessed the moment ANY `serp`/`browser`/`cdp` dispatch touches it, regardless of what the eval's own return value says -- `{"ready": false}`, `{"timedOut": true}`, `{"hasDebug": false}` all satisfy the mechanical gate exactly as well as a genuine `{"ready": true}` does, because the gate checks that a witness ran, not that it succeeded. This is a real, permanent gap in what the gate can mechanically enforce (the harness cannot know your assertion's success shape in advance), so the discipline has to hold where the gate cannot: **the moment your own eval's result shows the invariant did NOT hold -- `false` where you expected `true`, `timedOut: true`, an empty/null value for a global you expected populated -- that dispatch is a FAILED witness, not a completed one, full stop.** Do not read a negative result, note it mentally, and continue to other work "for now" -- the failure is the finding, and the very next dispatch must be root-causing it (read `stderr`/`debug.pageErrors`/`debug.console`, `codesearch` the failure point, fix, re-witness) before ANYTHING else, including unrelated fixes that feel more tractable in the moment. A session that gets distracted by a DIFFERENT problem (a crashing tool, a flaky daemon, an unrelated CI failure) after seeing a negative witness and never returns to resolve that specific negative result before declaring the turn done has produced exactly the forced-closure violation this section exists to prevent -- the distraction does not pause the obligation, it defers a debt that must be paid before the final DECIDE gate. If two consecutive witness attempts against the same invariant both come back negative, `prd-add` a row naming the concrete symptom (not "browser flaky" -- the actual thing that didn't boot/render/populate) and root-cause it as its own tracked item; a negative witness result is never silently subsumed into an unrelated PRD row about tooling instability.

Fires across phases: **PROVE** edit -> same-turn `serp`/`browser`/`cdp` dispatch asserting the invariant; **EMIT** post-emit re-witness (page still passes after the full diff); **DECIDE** final gate -- `deviation.browser-witness-hash-mismatch` fires if a witnessed file changed without re-witnessing. Pure-prose static-document edits (no JS, no CSS-driven behavior, no DOM mutation) are the ONLY exempt category, and the exemption must be named explicitly in the response so the skip is auditable. Silent skip on actual behavior change is forced closure.

YOU drive the browser through the spool: plugkit holds the engine handle (Chromium for `cdp`, lightpanda/steel for `browser`, in-process oxibrowser for `serp`), per-project profile, session table; advance by writing `.gm/exec-spool/in/<verb>/<N>.txt`, reading `out/<N>.json`. No library import, no puppeteer/playwright/CDP shortcut. The verb is the surface; every other reach = fabrication.
...
Profile to LOCATE (which call/resource is slow), then rule out hypotheses by live measurement -- never a/b-test by restarting. The node side mirrors this: `exec_js` with `process.hrtime.bigint()`/`performance.now()` timing, `process.memoryUsage()`, and `stderr` stack capture is a genuine node profiler+debugger.
...
Never spawn Chromium/lightpanda yourself, `npm i puppeteer`, or shell `chrome.exe`; the verb owns the handle, and bypassing it orphans state plugkit cannot reap and breaks the next session's first read. Navigate by evaluating `location.href = '...'` through the spool; screenshot by dispatching the verb that returns one. A dispatch returning `ok:false` with a launch error is plugkit reporting the environment refused -- read `stderr`, dispatch `instruction`, do not loop the same body.

**Dead-watcher recovery, never substitute.** If a Write to `.gm/exec-spool/in/<serp|browser|cdp>/<N>.txt` produces no response after two re-Reads AND `.status.json` ts is stale (>5min from current epoch), the watcher is dead: boot with `curl -fsSL https://raw.githubusercontent.com/AnEntrypoint/gm/main/install.sh | sh -s -- spool` (PowerShell: `irm https://raw.githubusercontent.com/AnEntrypoint/gm/main/install.ps1 | iex` then `agentplug-runner spool`), then re-dispatch the same body. Do NOT reach for puppeteer-core, puppeteer, playwright, agent-browser, `chrome.exe`, `npx browserless`, WebFetch, or curl-then-parse -- `serp`/`browser`/`cdp` are the only admissible browser surfaces; substitutes spawn orphan processes plugkit cannot reap, bypass section 23 witness-marked events, and produce evidence the gate cannot read. The recovery loop is always: empty response -> check `.status.json` -> if stale, boot -> re-dispatch.

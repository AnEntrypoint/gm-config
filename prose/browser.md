...
`browser` now runs a headless pure-Rust engine (oxibrowser) by default:
...
no-ops). Everything else in this document -- session-per-Chromium,
`screenshot`/`capture`/`profile`/`trace`, headed-by-default, CDP recipes --
describes `cdp`, the real-Chrome/playwright-style verb with the exact same
plain-text-body grammar. Any visual/rendering witness (the Hard Rule below)
REQUIRES `cdp`'s `screenshot` mode; `browser` cannot satisfy it and will
reject a `screenshot`/`capture`/`profile`/`trace` prefix with a note pointing
at `cdp`. For a plain evaluate/navigate/dom-query witness where visual
fidelity does not matter, `browser` is the faster default -- reach for `cdp`
whenever the task needs anything this paragraph names that `browser` does
not cover.

## Hard Rule: Browser Witness Mandate

**Every edit to browser-run code requires a live `browser` or `cdp` dispatch, same turn as the edit.** Client-side surfaces -- `.html`, `.js`, `.jsx`, `.ts`, `.tsx`, `.vue`, `.svelte`, `.mjs`, `.css`, web components, service workers, every `<script>`-loaded asset, every `import`-reached path from a browser entry -- witnessed by live `page.evaluate` of the specific invariant the edit establishes; use `cdp` where the invariant needs real Chrome fidelity or a screenshot, `browser` (oxibrowser) otherwise. Passing node test / build / `curl` of HTML / static-analysis witnesses server delivery, not browser behavior -- non-substitutive. The witness IS the proof; prose is not.
